...
# from utils.read_exif_data import parse_GPS_info_to_xyz
...
def draw_2D_arrs(datax, datay, rotate_x, rotate_y, saveFile):
    
    import numpy as np  
    import matplotlib.pyplot as plt  
    
    plt.scatter(datax, datay, color="blue")
    plt.scatter(rotate_x, rotate_y, color="red")
...
if __name__ == "__main__":
    ...
    file_input = "/home/xavision/nnd_storage_0/Asher/data/PV_wheat_all/9822e12f10cd92d001282218"
    file_output = "/home/xavision/nnd_storage_0/Asher/data/PV_wheat_all/9822e12f10cd92d001282218/rotate1"
    ...
    if _data_set_type == "PV":
        ...
        rotate_x, rotate_y = [], []
        for idx, img in enumerate(_image_lists):
            imgFile = os.path.join(file_input, img)
            assert os.path.isfile(imgFile)
            outFile = os.path.join(file_output, img)
            
            _fr = _flight_route[idx]
            if _fr % 2 != 0:
                print("旋转")
                rotate_img(imgFile, outFile)
                rotate_x.append(_ecef_x[idx])
                rotate_y.append(_ecef_y[idx])
            else:
                import shutil
                shutil.copy(imgFile, outFile)
                print("不旋转")
        
        saveFile = os.path.join(file_output, "check_rotate.png")
        draw_2D_arrs(_ecef_x, _ecef_y, rotate_x, rotate_y, saveFile)
